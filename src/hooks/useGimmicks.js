...
export function spawnConfetti(colors) {
  const cols = colors || ["#f05a28","#ff7a45","#fbbf24","#4ade80","#38bdf8","#f472b6"];
  for (let i = 0; i < 80; i++) {
...
function show404() {
  ...
  `;
  el.innerHTML = `
    <div style="font-size:clamp(80px,18vw,160px);font-weight:800;color:#f05a28;letter-spacing:-6px;line-height:1;">404</div>
    ...
    <div style="font-family:'Fira Code',monospace;font-size:11px;color:#f05a28;margin-top:32px;letter-spacing:2px;">// click anywhere to dismiss</div>
  `;
  document.body.appendChild(el);
...
function triggerRave() {
  ...
  setTimeout(() => {
    ...
    document.documentElement.style.setProperty("--accent2", "#ff7a45");
...
function showSecretMessage() {
  ...
  accentBar.style.cssText = `
    ...
    background:linear-gradient(90deg,#f05a28,#ff7a45,transparent);
    pointer-events:none;
  `;
  ...
  card.innerHTML += `
    <div style="position:relative;z-index:1;pointer-events:auto;">
      <div style="font-size:10px;font-family:'Fira Code',monospace;color:#f05a28;letter-spacing:3px;text-transform:uppercase;margin-bottom:20px;">
        // secret unlocked
      </div>
      ...
      <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
        <a href="mailto:krapa.kalyan@gmail.com" style="
          ...
          padding:13px 28px;border:1px solid #f05a28;cursor:none;
        ">✉️ Email Me →</a>
...
export function useGimmicks() {
  useEffect(() => {
    ...
    if (now.getMonth() === 6 && now.getDate() === 12) {
      setTimeout(() => {
        spawnConfetti(["#f05a28","#ff7a45","#fbbf24","#f472b6","#a855f7","#38bdf8"]);
