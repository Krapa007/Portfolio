...
export default function BackgroundCanvas() {
  ...
  useEffect(() => {
    ...
    const orbiters = [
      { a: 0,              spd: 0.009, rad: 200, sz: 220, op: 0.35 },
      { a: Math.PI * 0.66, spd: 0.013, rad: 150, sz: 170, op: 0.28 },
      { a: Math.PI * 1.33, spd: 0.007, rad: 250, sz: 280, op: 0.18 },
    ];
    ...
    const tick = () => {
      ...
      // 3. Cursor blob + orbiters
      {
        ...
        g.addColorStop(0, AR(isLt() ? 0.22 : 0.42));
        g.addColorStop(0.45, AR(isLt() ? 0.08 : 0.16));
      ...
      // 5. Ambient blobs
      [
        { bx: W * 0.85 + Math.cos(t * 0.52) * W * 0.06, by: H * 0.18 + Math.sin(t * 0.44) * H * 0.09, r: 240, o: isLt() ? 0.14 : 0.28 },
        { bx: W * 0.12 + Math.sin(t * 0.76) * W * 0.06, by: H * 0.82 + Math.cos(t * 0.58) * H * 0.07, r: 200, o: isLt() ? 0.12 : 0.22 },
        { bx: W * 0.5  + Math.cos(t * 0.33) * W * 0.08, by: H * 0.5  + Math.sin(t * 0.41) * H * 0.06, r: 180, o: isLt() ? 0.07 : 0.16 },
      ].forEach((b) => {
